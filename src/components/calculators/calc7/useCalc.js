...
export function useCalc() {
  ...
  const getStatus = (score) => {
    ...
    if (score <= 4) return 'yellow-green'
    if (score <= 6) return 'orange'
    return 'red'
  }

  const getMessage = (score) => {
    if (score < 2) return '三分法では…低リスク（1.3%）\n二分法では…PEの可能性が低い（12.1%）'
    if (score <= 4) return '三分法では…中リスク（16.2%）\n二分法では…PEの可能性が低い（12.1%）'
    if (score <= 6) return '三分法では…中リスク（16.2%）\n二分法では…PEの可能性が高い（37.1%）'
    return '三分法では…高リスク（37.5%）\n二分法では…PEの可能性が高い（37.1%）'
  }
